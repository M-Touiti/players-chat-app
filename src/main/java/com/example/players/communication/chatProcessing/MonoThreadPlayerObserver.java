 ...
 * Represents each Observer that is able to receive and monitor changes in the subject.
 * It takes in the parameter the chat and the players.
 * 
 * @author Mohamed TOUITI (mtouiti.info@gmail.com)
 */
public class MonoThreadPlayerObserver implements Observer {
     ...
     */
	public MonoThreadPlayerObserver(Subject chat, Player player) {
	...
	public void receive(Player sender, String msg) {
		 ...
		 * then save the received message to the message queue 
		...
		if(canWrite() && !this.receivedMessages.isEmpty()) {
			 ...
			 * and concatenate it with the number of messages this player already sent.
			 */
